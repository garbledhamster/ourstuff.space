      ...
      <div class="donation-panel">
        ...
        <div class="amount-grid" aria-label="Donation amount">
          ${[5, 10, 15, 20, 25, 50, 100]
						.map(
							(amount) =>
								`<button class="amount-option${amount === 10 ? " is-selected" : ""}" type="button" data-amount="${amount}">$${amount}</button>`,
						)
						.join("")}
        </div>
...
export function bindDonationFlow(root = document, options = {}) {
	...
	const openButtons = Array.from(
		root.querySelectorAll("[data-action='open-donation']"),
	);
	...
	async function startDonation(amount) {
		const response = await fetch(
			`${DONATION_WORKER_URL}/api/donations/checkout`,
			{
				method: "POST",
	...
	submitButton.addEventListener("click", async () => {
		try {
			...
			status.textContent =
				error instanceof Error ? error.message : "Donation checkout failed.";
			submitButton.disabled = false;
			submitButton.innerHTML = `${iconHtml("tabler:heart-handshake")}<span class="button-label">Continue to Checkout</span>`;
		}
	});
