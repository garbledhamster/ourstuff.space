...
export function normalizeTheme(value, { themes = [], fallbackId = "default" } = {}) {
...
export function loadTheme({
  ...
  try {
    return normalizeTheme(safeStorage(storage)?.getItem(storageKey), { themes, fallbackId });
  } catch {
...
export function themeById(themeId, { themes = [], fallbackId = "default" } = {}) {
  const normalized = normalizeTheme(themeId, { themes, fallbackId });
...
export function themeFonts(theme, { fontSets = DEFAULT_FONT_SETS, fallbackFontSet = "classic" } = {}) {
  return {
...
export function themeUsesLineContrast(theme) {
  return theme?.contrastMode === "line"
...
export function resolveThemeTokens(theme) {
  ...
  return {
    ...
    "--border": lineContrast ? colors.primaryColor : colorWithAlpha(colors.primaryColor, lightTheme ? 0.42 : 0.48),
    "--border-soft": colorWithAlpha(colors.primaryColor, lineContrast ? 0.58 : lightTheme ? 0.2 : 0.26),
    "--border-strong": lineContrast ? colors.primaryColor : colorWithAlpha(colors.primaryColor, lightTheme ? 0.72 : 0.78),
    "--border-muted": colorWithAlpha(colors.secondaryColor, lineContrast ? 0.46 : lightTheme ? 0.28 : 0.3),
    "--border-accent": colors.primaryColor,
    "--border-accent-soft": colorWithAlpha(colors.primaryColor, lineContrast ? 0.74 : lightTheme ? 0.54 : 0.6),
    "--theme-divider": colorWithAlpha(colors.secondaryColor, lineContrast ? 0.42 : lightTheme ? 0.24 : 0.22),
    ...
    "--theme-select-text": readableTextFor(colors.surfaceColor, colors.textColor),
    "--theme-select-option-bg": colors.surfaceMutedColor,
    "--theme-select-option-text": readableTextFor(colors.surfaceMutedColor, colors.textColor),
    "--theme-select-option-active": colors.primaryColor,
    "--theme-select-option-active-text": readableTextFor(colors.primaryColor, colors.textColor),
    "--theme-ring": colorWithAlpha(colors.primaryColor, 0.34),
    "--theme-shadow": colorWithAlpha(lightTheme ? colors.primaryColor : colors.backgroundColor, lightTheme ? 0.18 : 0.48),
    ...
    "--on-surface-2": readableTextFor(colors.surfaceMutedColor, colors.textColor),
...
export function applyThemeVariables(themeId, {
  ...
  themeClassNames(themes, classPrefix).forEach((className) => selectedRoot.classList.remove(className));
  selectedRoot.classList.toggle(paletteClass, Boolean(theme.colors));
  selectedRoot.classList.add(`${classPrefix}${theme.id}`);

  styleProperties.forEach((property) => selectedRoot.style.removeProperty(property));
...
export function createThemeController(options = {}) {
  let activeThemeId = normalizeTheme(options.initialTheme || loadTheme(options), options);
  return {
    get themeId() {
